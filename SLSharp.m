function [Xhat_im , output ] = SLSharp(Yim,varargin)
% Usage [ Xhat_im , output ] = SLSharp(Yim,varargin)
%
...
%save('/home/jakobs/s2/params/data/data.mat')
    ...
    % Sequence of bands
    % [B1 B2 B3 B4 B5 B6 B7 B8 B8A B9 B11 B12 L1-L8]
    ...
    mtfs2 = [ .32 .26 .28 .24 .38 .34 .34 .26 .33 .26 .22 .23 ];
    % We need Landsat std
    %D:\OneDrive - Háskóli Íslands\data\landsat8\createLandsat8PSF.m
    %D=[D'  30 30 30 30 30 30 30 30 10 20 ]';
    d=[d' 3 3 3 3 3 3 3 1 2]';
    mtf8=[0.4896    0.4884    0.4872    0.4843    0.4806    0.4615 0.4544 0.1 0.4612];
    mtf=[mtfs2 mtf8];
    sdf = d/pi.*sqrt(-2*log(mtf'));
    ...
    for jCD=1:CDiter
       ...
       if( GCV==1 )
            ...
            Ynoise = Ynoise(noiseIdx,:); 
            HtHBXnoise = HtHBXnoise(noiseIdx,:);
            den = trace(Ynoise*(Ynoise - HtHBXnoise)');           
            HtHBX=Mask.*ConvCM(F*Z,FBM,nl); 
            num = norm( Y(noiseIdx,:) - HtHBX(noiseIdx,:) , 'fro')^2;         
